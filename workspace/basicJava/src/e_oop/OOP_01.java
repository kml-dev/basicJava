		 ...
		 2. 클래스와 객체
		  ...
		  4. 선언위치에 따른 변수의 종류 
		    ...
		    - 클래스 변수
		      ...
		      : 인스턴스 변수 static만 붙이면 된다.
		    
		    - 지역변수
		      : 메서드 내에 선언되며 메서드 내에서만 사용 가능하다.
		      : 메서드 종료시 소멸된다.
		      : for문 또는 while문의 블럭내에서 선언된 변수는 해당 반복문이 종료되면 소멸된다.
		  
		  5. 메서드 
		    - 어떤 작업을 수행하기 위한 명령문의 집합
		    - 주로 어떤 값을 입력받아서 처리하고 그 결과를 돌려준다.
		    - 입력값이 없을 수도 결과를 반환하지 않을 수도 있다.
		    - 반복적으로 사용되는 코드를 줄이기 위해서 사용된다. => 코드양도 줄고 유지보수가 편리하다.
		    - 작성방법
		   ...
		   6. return문
		     ...
		     - 반환값
		      : 반환값이 있는 경우
		      -> return문  뒤에 반환값을 지정해 주면 된다.
		      -> 반환값의 타입과 메서드의 반환 타입이 같아야 한다.
		      : 반환값이 없는 경우
		      -> return;
		   ...
		   8. JVM메모리 구조
		     - 메서드 영역(Method Area)
		       : 클래스가 로드되며 클래스 변수와 클래스 메서드가 함께 로드된다.
		     - 호출 스택(Call stack)
		       : 메서드 작업에 필요한 메모리 공간을 제공한다.
		     - 힙 영역(Heap Area)
		       : 인스턴스가 저장된다.
		       : 인스턴스 변수와 인스턴스 메서드가 함께 로드된다.
		   ...
		   11. 메서드 오버로딩
		     ...
		     - 오버로딩의 장점
		       ...
		       : 근본적으로 같은 기능을 하는 것인데 다름 이름으로 표현되면 기억하기 어렵다.
		   ...
		   13. 생성자
		     ...
		     - 특징
		       : 메서드처럼 클래스내에 선언된다.
		       : 생성자가 없을 때만 만들어 줌
		       : 반환값이 없다. => 생성자도 메서드이기 때문에 void를 적어주어야 하지만
