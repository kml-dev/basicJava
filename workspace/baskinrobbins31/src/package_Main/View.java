...
public class View {
	...
	private void orderInfoView() {
		List<OrderInformationVO> orderList = iServiceImpl.selectAllOrderInfomation();
		String message = "";
		while (true) {
			System.out.println("⊂==============================================================================⊃");
			if (orderList.size() > 0) {
				for (int i = 0; i < orderList.size(); i++) {
					...
					UserVO user = iServiceImpl.selectUser(orderInformation.getUser_id());
					SizeVO size = iServiceImpl.selectSize(orderInformation.getSize_seq());
	...
	private void orderInfoDetails(int order_seq) {
		while (true) {
			OrderInformationVO order = iServiceImpl.selectOrderInfomation(order_seq);
			List<OrderDetailsVO> detail = iServiceImpl.selectOrderDetails(order.getSeq());
			...
			if (orderInformation.getSpoonCount() == -1) {
				...
				for (int i = 0; i < size.getFlavorKinds(); i++) {
					if (i < orderDetailsList.size()) {
						System.out.println(i + 1 + "번째 맛 : " + iServiceImpl.selectIcecream(orderDetailsList.get(i).getIcecream_seq()).getKinds());
	...
	private void orderHistoryView() {
		while (true) {
			List<OrderInformationVO> orderList = iServiceImpl.selectAllOrderInformation(user.getId());
			if (orderList.size() == 0) {
				System.out.println("주문 내역이 없습니다.");
			}
			for (int i = 0; i < orderList.size(); i++) {
				SizeVO size = iServiceImpl.selectSize(orderList.get(i).getSize_seq());
				System.out.println();
	...
	private int refund(int orderInfo_seq) {
		...
		// 유저에게 돈 돌려주기
		OrderInformationVO order = iServiceImpl.selectOrderInfomation(orderInfo_seq);
		SizeVO size = iServiceImpl.selectSize(order.getSize_seq());
